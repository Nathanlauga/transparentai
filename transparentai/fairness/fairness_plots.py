...
def get_metric_text(protected_attr, target_value, metric_name, bias_type='dataset'):
    ...
    freq_unpr = get_protected_attr_freq(
        protected_attr, target_value, privileged=False, predictions=True)
    freq_priv = get_protected_attr_freq(
        protected_attr, target_value, privileged=True, predictions=True)
...
def plot_model_bias_metrics(protected_attr, target_value):
    ...
    for idx, metric in enumerate(metrics):
        axes = generate_gridspec_row(
            fig, gs, rstart=idx, rsize=1, n_blocks=2, b_sizes=[4, 6])
        plot_bias_metric(axes, protected_attr, target_value,
...
def plot_bias_metrics(protected_attr, bias_type='dataset', target_value=None):
    ...
    if target_value is not None:
        display(
            Markdown(f'### Focus on {attr} for {target} is {target_value}'))
